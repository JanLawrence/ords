$(function(){
    ...
    $('#researchList').on('click', '.btn-upload', function(){ // on click notes buttons on research list
        var id = $(this).attr('rid'); // get attr values for status and research id
        $('#uploadResearchForm').find('input[name="id"]').val(id); // put attr values on each specific input 
        
        $('#uploadResearchModal').modal('toggle'); // toggle notes modal
    })
    $('#uploadResearchForm').submit(function(){ // on click notes buttons on research list
        var file = $('#uploadResearchForm').find('input[name="file"]').get(0).files.length; // get file length
        var form = new FormData(this); // get form data\
 
        if(file != 0){ // if neither two of content or file is required
             $.ajax({
                 url: URL + 'research/upload', // post to research/add
                 type: "POST",
                 data:  form,
                 ...
                 success: function(returnData){ // get returned data in the posted link by using returnData variable
                     if(returnData == 2){ // if returned data is equal to 2 or if not PDF
                         ...
                         alert('Research successfully uploaded!')
            ...
            $.ajax({
                ...
                success: function(returnData){ // get returned data in the posted link by using returnData variable
                    if(returnData == 2){ // if returned data is equal to 2 or if not PDF
                        ...
                        alert('Research successfully submitted!')
                        location.href = URL+"research/researchList"; //if success redirect to research list
                    }
                }
            });
